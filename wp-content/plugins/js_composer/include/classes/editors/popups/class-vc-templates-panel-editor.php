...
class Vc_Templates_Panel_Editor {
  ...
  $arr_category = array(
     ...
     /*khoapq addition*/
     'category_name' => esc_html__( 'Thim Templates', 'js_composer' ),
     /*khoapq addition end*/
  ...
  foreach ( $default_templates as $template_id => $template_data ) {
 	...
 	// preview image
        /*khoapq addition*/
        'cat'          => isset( $template_data['cat'] ) ? $template_data['cat'] : '',
        /*khoapq addition end*/ 
  ...
  if ( ! empty( $category_templates ) ) {
	...
	public function renderTemplateListItem( $template ) {
     ...
     /*khoapq addition*/
     $preview_img = esc_attr( isset( $template['image'] ) && $template['image'] != '' ? $template['image'] : get_template_directory_uri() . '/templates/no-img.jpg' );
     $cat         = esc_attr( isset( $template['cat'] ) ? $template['cat'] : '' );
     $cat_filter  = 'cat-filter-all cat-filter-' . sanitize_title( $cat );
     /*khoapq addition end*/

     /*khoapq addition - $cat_filter */
     $output = <<<HTML
              <div class="vc_ui-template vc_templates-template-type-$template_type $custom_class $cat_filter"
...
HTML;
     ...
     if ( ! empty( $preview_img ) && $template_type == 'default_templates' ) {
        $output .= '<div class="preview-image"><img src="' . $preview_img . '" alt="' . $name . '"  /></div>';
     }
     /*khoapq addition end*/
