...
if ( esc_attr( $category['category'] ) == 'default_templates' ) {
  echo '<div class="cat-filter">
     <ul>';

  echo '<li data-filter="cat-filter-all" class="active">' . esc_attr__( 'All', 'js_composer' ) . '</li>';

  $cat_filter = array();
  foreach ( $category['templates'] as $template ) {
     $cat_filter[ sanitize_title( $template['cat'] ) ] = $template['cat'];
  }

  foreach ( $cat_filter as $slug => $name ) {
     echo '<li data-filter="cat-filter-' . $slug . '">' . $name . '</li>';
  }
  echo '</ul>
  </div>';
}
