  ...
  $.fn.lpListAnnouncements = function (opts) {
    ...
    var LPListAnnouncements = {
      ...
      events: function () {
        ...
        $('.lp-send-mail, #lp-display-comment', _this.$el).change(function () {
        ...
        _this.$el.on('search', '.lp-course-search', function () {
        ...
        _this.$listItem.change(function () {
          ...
          if (ids.length) {
            _this.$buttonAdd.text(
              _this.$buttonAdd.data('text') + ' (+' + ids.length + ')').
              removeAttr('disabled')
            _this.$buttonAddClose.text(
              _this.$buttonAddClose.data('text') + ' (+' + ids.length + ')').
              removeAttr('disabled')
          }
          else {
            _this.$buttonAdd.text(_this.$buttonAdd.data('text')).
              attr('disabled', 'disabled')
            _this.$buttonAddClose.text(_this.$buttonAddClose.data('text')).
              attr('disabled', 'disabled')
          }
        })
        ...
        _this.$listCourseSelected.on('selectedCourses', function () {
          ...
          $.each(ids, function (index, item) {
            ...
            var $template = _this.$templateCourse.clone().
                removeClass('lp-hidden'),
              url = ajaxurl.split('admin-ajax.php')[0] + 'post.php?post=' +
                item + '&action=edit'
        ...
        _this.$tbody.on('click', '.lp-remove', function (event) {
        ...
        _this.$el.on('click', '.remove-items-announcements', function (event) {
          ...
          $('.lp_announcement-item', _this.$tbody).
            not('.lp-hidden').
            each(function () {
        ...
        $('.lp-check-all-items', _this.$el).change(function () {
          ...
          $('.lp_announcement-item', _this.$tbody).
            not('.lp-hidden').
            find('.item-checkbox input[type="checkbox"]').
            each(function () {
              $(this).prop('checked', checked)
              $(this).trigger('change')
            })
      ...
      createAnnouncement: function () {
        ...
        $('.lp-course-item-select', _this.$listCourseSelected).
          not('.lp-hidden').
          each(function () {
      ...
      sendMail: function () {
        ...
        _this.$el.on('click', '.lp-send', function (event) {
          ...
          if ($(this).hasClass('lp-ajax-loading') ||
            $(this).hasClass('lp-active')) {
            return
          }
          ...
          $.ajax({
            ...
            data: {
              ...
              announcement_id: $self.closest('.lp_announcement-item').
                attr('data-id'),
              course_id: course_id,
            },
            type: 'POST',
      ...
      loadCourse: function () {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (ids) {
              ...
              $.each(ids, function () {
                ...
                var $item = $('.lp-announcement-item[data-id="' + this + '"]',
                  _this.$listItem)
          ...
          error: function () {
            _this.$listItem.append(
              '<li>OOP! Something has gone wrong, please try again.</li>')
          },
          complete: function () {
