...
if ( ! class_exists( 'LP_Addon_Assignment' ) ) {
	...
	class LP_Addon_Assignment extends LP_Addon {
		...
		public function __construct() {
			...
			add_action( 'learn-press/course-section-item/before-lp_assignment-meta',
				array( $this, 'learnpress_assignment_show_duration' ), 10 );
			add_action( 'learn-press/course-section-item/before-lp_assignment-meta',
				array( $this, 'learnpress_assignment_meta_final' ), 15 );
			...
			add_filter( 'learn-press/evaluate_passed_conditions', array( $this, 'learnpress_assignment_evaluate' ), 10,
				3 );
			add_filter( 'learn-press/get-course-item', array( $this, 'learnpress_assignment_get_item' ), 10, 3 );
			add_filter( 'learn-press/default-user-item-status',
				array( $this, 'learnpress_assignment_default_user_item_status' ), 10, 2 );
			add_filter( 'learn-press/user-item-object', array( $this, 'learnpress_assignment_user_item_object' ), 10,
				2 );
			add_filter( 'learn-press/course-item-type', array( $this, 'learnpress_assignment_course_item_type' ), 10,
				1 );
			add_filter( 'learn-press/block-course-item-types',
				array( $this, 'learnpress_assignment_block_course_item_type' ), 10, 1 );
			...
			add_filter( 'learn-press/course-item/completed',
				array( $this, 'learnpress_assignment_count_evaluated_item' ), 10, 3 );

			// add passed or failed class when display item:
			add_filter( 'learn-press/course-item-status-class', array( $this, 'learnpress_assignment_add_class_css' ),
				10, 3 );
			...
			add_action(
				...
				function ( $privacy ) {
					$privacy[] = array(
						'name'        => esc_html__( 'Assignments', 'learnpress' ),
						'id'          => 'assignments',
						'default'     => 'no',
						'type'        => 'yes-no',
						'description' => esc_html__( 'Public your profile Assignment.', 'learnpress' ),
					);
			...
			add_filter( 'learn-press/content-drip/item-status', array( $this, 'update_status_item_content_drip' ), 10,
				4 );

			if ( is_plugin_active( 'learnpress-co-instructor/learnpress-co-instructor.php' ) ) {
				add_filter( 'learn-press/co-instructor/case-post-type-can-edit',
		...
		public function frontend_editor_enqueue() {
			wp_enqueue_style(
				'learn-press-assignment-fe',
				plugins_url( '/assets/css/assignment-front-editor.css', LP_ADDON_ASSIGNMENT_FILE )
			);

			wp_enqueue_script(
				'learn-press-assignment',
				plugins_url( '/assets/js/assignment-front-editor.js', LP_ADDON_ASSIGNMENT_FILE ),
				array(
		...
		public function learnpress_update_final_assignment( $post_id ) {
			if ( LP_COURSE_CPT === get_post_type( $post_id ) ) {
				...
				if ( isset( $final_assignment ) ) {
					update_post_meta( $post_id, '_lp_final_assignment', $final_assignment );
		...
		public function emails_setting( &$emails, $obj ) {
			...
			$emails['LP_Email_Assignment_Submitted_Instructor'] = include 'emails/class-lp-email-submitted-assignment-instructor.php';
			...
			$emails['LP_Email_Assignment_Evaluated_Instructor'] = include 'emails/class-lp-email-evaluated-assignment-instructor.php';
		...
		public function learnpress_assignment_evaluate( $results, $course_result, $user_course ) {
			if ( ! is_object( $user_course ) || ! isset( $user_course ) || ! $user_course->is_enrolled() ) {
				return false;
			}
		...
		public function assignment_enqueue_scripts() {
			if ( function_exists( 'learn_press_is_course' ) && learn_press_is_course() ) {
				wp_enqueue_script( 'lp-plugins-all' );

				wp_enqueue_style(
					'learn-press-assignment',
					plugins_url( '/assets/css/assignment.css', LP_ADDON_ASSIGNMENT_FILE )
				);

				if ( learnpress_is_assignment_page() ) {
					wp_enqueue_script(
						'learn-press-assignment',
						plugins_url( '/assets/js/assignment.js', LP_ADDON_ASSIGNMENT_FILE ),
						array(
							'jquery',
							'plupload-all',
						)
		...
		/**
		 * Override can view item's course
		 *
		 * @param Model_User_Can_View_Course_Item|boolean $can_view
		 * @param int                                     $item_id
		 * @param int                                     $user_id
		 * @param int                                     $course_id
		 *
		 * @editor tungnx
		 * @return mixed|Model_User_Can_View_Course_Item|boolean
		 * @deprecated   4.0.0
		 */
		/*
		public function learnpress_assignment_can_view_item( $can_view, $item_id, $user_id, $course_id ) {
			if ( 'lp_assignment' === get_post_type( $item_id ) ) {
				$course = false;
				$user   = learn_press_get_user( $user_id );
		...
		public static function process_assignment() {
			...
			if ( ! $pressed_button ) {
				$pressed_button = 'Send';
				$user_note      = __(
					'This is the result which was forced to send because of time up issue',
					'leanrpress-assignments'
				) . $user_note;
			}
...
//				learn_press_update_user_item_meta( $current_useritem_id, '_lp_assignment_answer_note', $user_note );
				$assignment_db->update_extra_value( $current_useritem_id, LP_Assigment_DB::$answer_note_key, $user_note );
				...
				if ( isset( $_FILES['_lp_upload_file'] ) && $_FILES['_lp_upload_file']['name'][0] != '' ) {
					...
					foreach ( $uploadedfiles['name'] as $key => $value ) {
						if ( $total_files >= $allow_file_amount ) {
							$result['message'] .= __(
								' Your uploaded files reach the maximum amount!',
								'learnpress-assignments'
							);
							break;
						}
						...
						if ( $file['size'] > $max_upload_size * 1024 * 1024 ) {
							$result['message'] .= sprintf(
								__(
									" The size of your %1\$s file is over %2\$d Mb(s).\n",
									'learnpress-assignments'
								),
								$file['name'],
								$max_upload_size
						...
						if ( $movefile && ! isset( $movefile['error'] ) ) {
							...
							$result['message'] .= ' ' . $movefile['error'] . sprintf(
								__(
									' Please check file %s!',
									'learnpress-assignments'
								),
								$uploadedfiles['name'][ $key ]
							);
						}
					}

					remove_filter( 'upload_dir', array( __CLASS__, 'learnpress_assignment_upload_dir' ) );
//					learn_press_update_user_item_meta( $current_useritem_id, '_lp_assignment_answer_upload', $uploaded_files );

					$assignment_db->update_extra_value( $current_useritem_id, $assignment_db::$answer_upload_key,
						json_encode( $uploaded_files ) );

					if ( $file_uploaded == count( $uploadedfiles['name'] ) ) {
						$result['message'] = __(
							'The progress was save! Your file(s) were uploaded successfully!',
							'learnpress-assignments'
						);
					} else {
				...
				if ( 'Send' == $pressed_button ) {
					...
					$evaluate_author = learn_press_get_user_item_meta(
						$current_useritem_id,
						'_lp_assignment_evaluate_author',
						true
					);
					...
					learn_press_update_assignment_item(
						$assignment_id,
						$course_id,
						$user,
						'completed',
			...
			if ( ! empty( $result['message'] ) ) {
				if ( $result['result'] == 'error' ) {
					$result['message'] = __(
						'Opps, something wrong while processing! ',
						'learnpress-assignments'
					) . $result['message'];
				}
		...
		public static function start_assignment() {
			...
			if ( ! empty( $result['message'] ) ) {
				$message_flag = 'error' === $result['result'] ? 'error' : 'success';

				learn_press_add_message( $result['message'], $message_flag );
			}
		...
		public function admin_assets() {
			...
			wp_enqueue_style(
				'learn-press-assignment',
				plugins_url( '/assets/css/admin-assignment.css', LP_ADDON_ASSIGNMENT_FILE )
			);
			wp_enqueue_script(
				'learn-press-assignment',
				plugins_url( '/assets/js/admin-assignment.js', LP_ADDON_ASSIGNMENT_FILE ),
				array( 'jquery' )
			);
