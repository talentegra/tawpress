...
function url_normalize(event) {
  try {
    ...
    const promoQueries = [ 'fbclid', 'fb_action_ids', 'fb_action_types', 'fb_source', '_ga', 'age-verified', 'ao_noptimize', 'usqp', 'cn-reloaded', 'klaviyo', 'amp', 'gclid', 'utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term' ]

    // Loop through the promo queries and see if we have any of these queries present in the URL, if so remove them
    promoQueries.forEach( (queryParam) => {
...
async function handleRequest(event) {
  ...
  if (!bypassCache && accept) {
    ...
    // Check if the accept type includes text/html i.e. the main HTML request
