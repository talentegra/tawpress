			...
			<div class="row align-items-center">
				<div class="col-lg-6 col-12">
					<p class="footer-copy text-lg-left text-center">
						<?php echo blockchain_lite_sanitize_footer_text( sprintf( __( 'A theme by <a href="%s" rel="nofollow">CSSIgniter</a>', 'blockchain-lite' ),
...
function blockchain_lite_get_default_footer_text( $position = 'left' ) {
	if ( 'right' === $position && get_theme_support( 'blockchain-lite-footer-text-right' ) ) {
		...
		if ( ! defined( 'BLOCKCHAIN_LITE_WHITELABEL' ) || ! BLOCKCHAIN_LITE_WHITELABEL ) {
			/* translators: %s is a url. */
			$text = sprintf( __( 'A theme by <a href="%s" rel="nofollow">CSSIgniter</a>', 'blockchain-lite' ),
				esc_url( 'https://www.cssigniter.com/' )
			);
		} else {
			/* translators: %1$s is a url. %2$s is the site's name. */
		...
		<nav class="nav">
			<?php
				...
				wp_nav_menu( array(
					...
					'fallback_cb'    => 'blockchain_lite_main_menu_fallback',
					'container'      => '',
...
function blockchain_lite_the_site_identity() {
	...
	if ( has_custom_logo() && get_theme_mod( 'show_site_title', 1 ) ) {
		...
		?><p class="site-logo"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></p><?php
	} elseif ( has_custom_logo() ) {
		?><p class="site-logo"><?php the_custom_logo(); ?></p><?php
	...
	if ( get_theme_mod( 'show_site_description', 1 ) ) {
		...
		if ( $description || is_customize_preview() ) {
			?><p class="site-tagline"><?php echo $description; // phpcs:ignore WordPress.Security.EscapeOutput ?></p><?php
		}
	}
