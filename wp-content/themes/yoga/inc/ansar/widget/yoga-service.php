...
class yoga_service_widget extends WP_Widget {    
                ...
                <div class="yoga-service-inner">
                   ...
                   <?php if ( !empty($instance['service_desc']) ): ?>
                    ...
                    <?php if ( !empty($instance['btnlink']) ): ?>
                        <a class="btn btn-theme-three" href="<?php echo esc_url($instance['btnlink']); ?>" target="<?php echo esc_html($businessup_btn_target); ?>"><?php echo esc_html($instance['btnmore']); ?></a>
                    <?php endif; ?>  
                </div>
            </div>
        </div>
	...
	?>
				...
				<td style="margin-bottom: 30px;">
                    <label for="<?php echo $this->get_field_id('service_title'); ?>"><?php esc_html_e('Title', 'yoga'); ?></label>
                </td>
				<td>
				...
				<td>
                    <label for="<?php echo $this->get_field_id('service_desc'); ?>"><?php esc_html_e('Description', 'yoga'); ?></label>
                </td>
				<td>
				...
				<p>
            <label for="<?php echo $this->get_field_id('image_uri'); ?>"><?php esc_html_e('Image', 'yoga'); ?></label><br/>
        ...
        <p>
            <input type="checkbox" name="<?php echo $this->get_field_name('hide_image'); ?>" id="<?php echo $this->get_field_id('hide_image'); ?>" <?php if( !empty($instance['hide_image']) ): checked( (bool) $instance['hide_image'], true ); endif; ?> ><?php esc_html_e( 'Hide Featured Image','yoga' ); ?>
            </p>
		<table>
        <tr>
        		<td>
        			<label for="<?php echo $this->get_field_id('btnmore'); ?>"><?php esc_html_e('Button Text', 'yoga'); ?></label>
        		</td>
        		<td>
        			<label for="<?php echo $this->get_field_id('btnlink'); ?>"><?php esc_html_e('Button Link', 'yoga'); ?></label>
        		</td>
        	</tr>
        	<tr>
        		<td>
        	...
        	<tr>
        		<td colspan="2">
        			<input type="checkbox" name="<?php echo $this->get_field_name('open_new_window'); ?>" id="<?php echo $this->get_field_id('open_new_window'); ?>" <?php if( !empty($instance['open_new_window']) ): checked( (bool) $instance['open_new_window'], true ); endif; ?> ><?php esc_html_e( 'Open link in a new tab','yoga' ); ?>
        		</td>
        	</tr>
		</table>	
    <?php
